...
@implementation BSCalendar

+ (BSCalendar *)share
{
    static dispatch_once_t once;
    static BSCalendar * share;
    dispatch_once(&once, ^{
        share = [self new];
        // Configure smth
    });
 ...
 */
- (long)jdFromDate:(int)dd month:(int)mm year:(int)yy {
    long a = (14 - mm) / 12;
    long y = yy + 4800 - a;
    long m = mm + 12 * a - 3;
    long jd = dd + (153 * m + 2) / 5 + 365 * y + y / 4 - y / 100 + y / 400
    - 32045;
    if (jd < 2299161) {
...
- (int)getLunarMonth11:(int)yy TimeZone:(double)timeZone {
    ...
    int sunLong = [self INT:([self getSunLongitude:nm TimeZone:timeZone])];
    if (sunLong >= 9) {
...
- (NSArray *)to_am:(int)dd month:(int)mm year:(int)yy TimeZone:(double)timeZone {
    ...
    long dayNumber = [self jdFromDate:dd month:mm year:yy];
...
- (NSMutableArray *)convertSolar2Lunar:(int)dd thang:(int)mm nam:(int)yy timeZone:(double)timeZone {
    int lunarDay, lunarMonth, lunarYear, lunarLeap;
    ...
    lunarDay = dayNumber-monthStart+1;
    long diff = (long)((monthStart - a11)/29);
    lunarLeap = 0;
    lunarMonth = diff+11;
    if (b11 - a11 > 365) {
